...
export class Ws {
  ...
  createListening() {
    ...
    this.ws.on("message", (data: wsResData) => {
      ...
      // 心跳测试
      if (
        wsRes.op === OpCode.HEARTBEAT_ACK ||
        wsRes.t === SessionEvents.RESUMED
      ) {
        if (!this.alive) {
          this.alive = true;
  ...
  getValidIntentsType(): AvailableIntentsEventsEnum[] {
    ...
    // 全部可监听事件
    const defaultIntents = Object.keys(
      AvailableIntentsEventsEnum
    ...
    if (intentsIn.length > defaultIntents.length) {
      BotLogger.info(
        "[CLIENT] intents中的监听事件大于可监听数，仅开启有效监听事件"
      );
    }
    // 如果intents中数据格式不对
  ...
  checkShards(shardsArr: Array<number> | undefined) {
    ...
    // 传进来的符合要求
    if (
      Array.isArray(shardsArr) &&
      shardsArr.length === 2 &&
      shardsArr[0] < shardsArr[1]
    ) {
      return shardsArr;
    }
